...
namespace WhippedCream
{
    ...
    public class RepositoryContextAttribute : System.Attribute
	{
		/// <summary>
		///		<para>
		///			This method queries the type specified by <typeparamref name="TInterface"/> for instances
		///			of <see cref="RepositoryContextAttribute"/> and returns a mapping based on the results.
		///		</para>
		/// 
		///		<para>
		///			There can be multiple instances of <see cref="RepositoryContextAttribute"/> on your repository.
		///			However, if there are multiple <see cref="RepositoryContextAttribute"/> attributes with the same
        ...
        public static IDictionary<DataLayerState, System.Type> GetRepositoryContexts<TInterface>()
        {
			...
			if (targetType.IsDefined(typeof(RepositoryContextAttribute), false))
            {
				IEnumerable<RepositoryContextAttribute> attributes =
					targetType.GetCustomAttributes(typeof(RepositoryContextAttribute), false).Cast<RepositoryContextAttribute>();
		...
		public RepositoryContextAttribute(DataLayerState state, System.Type repositoryType)
        {
            State = state;
